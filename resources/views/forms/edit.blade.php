        ...
        <form action="{{ route('forms.update', $form->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description"
                          class="w-full border border-gray-400 px-2 py-1">{{ old('description', $form->description) }}</textarea>
            </div>

            <div class="mb-4">
                <label for="form_json" class="block">Form JSON (SurveyJS):</label>
                <textarea name="form_json" id="form_json"
                          class="w-full border border-gray-400 px-2 py-1">{{ old('form_json', $form->form_json) }}</textarea>
            </div>

            <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded">Update Form</button>
        </form>
    </div>
</x-app-layout>
